...
L'architettura dell'applicazione è stata progettata seguendo il pattern architetturale a layer, in particolare è stata utilizzata una architettura a 3 layer, ovvero:
...
Questo layer è costituito da due elementi:
- L'*interfaccia utente* che consente di visualizzare i dati sullo schermo, rappresentata dai composable di _Jetpack Compose_, che insieme creano l'interfaccia grafica.

- I *titolari di stato*, cioè i ViewModel che contengono i dati, li espongono all'interfaccia utente e gestiscono la logica, i titolari di stato all'loro interno contengono lo stato della view che viene osservato dall'interfaccia UI e quindi aggiorna l'interfaccia non appena lo stato viene modificato.

=== Livello dati

Il livello dati di un'app contiene la logica di business e si occupa ci come l'app crea, archivia e modifica i dati.

Il livello dati è costituito dai repository che contengono due sorgenti dati, il database locale Room e il back-end remoto AWS: per una migliore esperienza anche in assenza di connessione, i dati vengono prima caricati dal database locale e poi sincronizzati con il back-end remoto.

=== Livello di dominio

Il livello di dominio è un livello posto in mezzo tra il livello dati e il livello di presentazione, è responssabile dell'incaplusamento di una logica di business più complessa o di logica che viene riutilizzata su più schermate. In questo livello si trovano le classi _UseCase_ che sono responsabili ciascuna di una _singola_ funzionalità.

Nel nostro caso viene anche utilizzato per dare valore aggiunto ad alcune funzionalità, come ad esempio l'inclusione del autore (o _host_ in qualche altra parte) nei dettagli di una location.
